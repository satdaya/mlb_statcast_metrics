...
cte_consolidation AS (
  SELECT 
    ...
    ,MAX(fb_velo)
    ,MAX(fb_spin_rate)
    ,MAX(fb_x_axis_movement)
    ,MAX(fb_y_axis_movement)
    ,MAX(br_velo)
    ,MAX(br_spin_rate)
    ,MAX(br_x_axis_movement)
    ,MAX(br_y_axis_movement)
